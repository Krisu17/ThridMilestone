...
def add_waybill():
    user = getUserFromCookie();
    isEmptyCookie = user is not None
    if isEmptyCookie:
        kurier_user = db.hget(user, "password")
        isValidCookie = kurier_user is not None
        if isValidCookie:
...
def pickup():
    login = getUserFromCookie() + "_kurier"
...
def show_wayshow_packagesbills():
    user = getUserFromCookie();
    isValidCookie = user is not None
    if isValidCookie:
